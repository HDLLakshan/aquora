...
import { SocietyCreateSchema } from "@aquora/shared";

import { createSociety } from "../../../lib/api/client";
import { FormInput } from "../../../components/form/FormInput";
import { FormSubmitButton } from "../../../components/form/FormSubmitButton";

const CreateSocietyFormSchema = z.object({
...
type CreateSocietyFormValues = z.output<typeof CreateSocietyFormSchema>;

type CreateSocietyPayload = z.infer<typeof SocietyCreateSchema>;
...
export function CreateSocietyForm() {
  ...
  const onSubmit = async (values: CreateSocietyFormValues) => {
    if (!data?.accessToken) {
    ...
    let payload: CreateSocietyPayload;
    try {
    ...
    try {
      await createSociety(data.accessToken, payload);
      reset();
      router.refresh();
    } catch {
      setError("root", { message: "Unable to create the society." });
    }
  };
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        {errors.billingSchemeJson ? (
          ...
          <p className="text-xs text-slate-500">Paste the billing rules JSON.</p>
        )}
      </div>
      ...
      <FormSubmitButton label="Create society" loadingLabel="Creating" isSubmitting={isSubmitting} />
    </form>
  );
}
